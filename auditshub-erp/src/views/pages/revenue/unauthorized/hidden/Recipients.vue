...
export default {
	...
	data () {
		return {
			...
			pkey: 'hidden-unauthorized-list-key',
			message: '',
			numbering: 0,
			currentPage: 1,
			result_per_page: 20,
	...
	mounted () {
		this.currentPage = Number(mStorage.get(`${this.pkey}page${this.accountid}`)) || 1
		this.getData()
	},
	watch: {
		currentPage () {
	...
	methods: {
		...
		getData (scroll) {
			...
			this.loading = true
			this.post('/hiddenunauthorized/recipients/', {
		...
		unhideWarn (account_id, offset_account) {
			Swal.fire({
				title: 'Are you sure?',
				...
				if (result.isConfirmed) {
					this.unhide(account_id, offset_account)
				}
			})
		},
		unhide (account_id, offset_account) {
			this.showLoading('Showing unauthorized Infraction, please wait')
			this.post('/hiddenunauthorized/group_unhide', {
				account_id,
				offset_account
			})
				.then((response) => {
