      ...
      <header class="header-table vs-table--header my-3">
        ...
        >
          <vs-button
            ...
            v-if="canAdd()"
            icon="icon-plus"
            v-on:click="linkto('/petroleume/tax-type/add')"
            >Add Tax Type</vs-button
          >
          <vs-button
            class="mx-2"
            color="danger"
          ...
          <table class="vs-table vs-table--tbody-table">
            ...
            <tbody>
              <tr
                ...
                :key="index"
                v-on:click="linkto('/petroleume/tax-type/' + record.id)"
              ...
              >
                ...
                <td>
                  <feather-icon
                    ...
                    v-if="canUpdate()"
                    @click.stop="linkto('/petroleume/tax-type/' + record.id)"
                  />
                  <feather-icon
                    icon="TrashIcon"
...
export default {
  ...
  methods: {
      ...
      this.post("/taxtype/", {
        ...
        .then((response) => {
          ...
          if (response.data.success == true) {
            this.message = "";
            this.records = response.data.taxes;
          } else {
            this.records =[]
    ...
    delete: function (ids) {
      this.showLoading("Deleting tax type(s), hang on a bit...");
      this.post("/taxtype/delete", {
        id: ids,
      })
        .then((response) => {
          this.closeLoading();
          if (response.data.success == true) {
            Swal.fire("Deleted!", "The tax type(s) has been deleted.", "success");
