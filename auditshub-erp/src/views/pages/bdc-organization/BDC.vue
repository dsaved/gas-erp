<template>
  <vx-card title="Bulk Distribution Companies">
    <p></p>
      ...
      <header class="header-table vs-table--header my-3">
        ...
        >
          <vs-button
            ...
            v-if="canAdd()"
            icon="icon-plus"
            v-on:click="linkto('/accounts/bdc-org/add')"
            >Add DBC</vs-button
          >
          <vs-button
            class="mx-2"
          ...
          <table class="vs-table vs-table--tbody-table">
            ...
            <tbody>
              <tr
                ...
                :key="index"
                v-on:click="linkto('/accounts/bdc-org/' + record.id+'/view')"
              ...
              >
                ...
                <td>
                  <feather-icon
                    ...
                    v-if="canUpdate()"
                    @click.stop="linkto('/accounts/bdc-org/' + record.id)"
                  />
                  <feather-icon
                    icon="TrashIcon"
...
export default {
	...
	methods: {
			...
			this.post('/bdc/', {
				...
				.then((response) => {
					...
					if (response.data.success == true) {
						this.message = ''
						this.records = response.data.omcs
					} else {
		...
		delete (ids) {
			...
			this.post('/bdc/delete', {
				id: ids
			})
				.then((response) => {
					this.closeLoading()
					if (response.data.success == true) {
						Swal.fire('Deleted!', 'The product(s) has been deleted.', 'success')
