      ...
      <header class="header-table vs-table--header my-3">
        ...
        >
          <vs-button
            ...
            v-if="canAdd()"
            icon="icon-plus"
            v-on:click="linkto('/organizations/add')"
            >Add Organization</vs-button
          >
          <vs-button
            class="mx-2"
            color="danger"
          ...
          <table class="vs-table vs-table--tbody-table">
            ...
            <tbody>
              <tr
                ...
                :key="index"
                v-on:click="linkto('/organizations/' + record.id)"
              ...
              >
                ...
                <td>
                  <feather-icon
                    ...
                    v-if="canUpdate()"
                    @click.stop="linkto('/organizations/' + record.id)"
                  />
                  <feather-icon
                    icon="TrashIcon"
...
export default {
	...
	methods: {
			...
			this.post('/organizations/', {
				...
				.then((response) => {
					...
					if (response.data.success == true) {
						this.message = ''
						this.records = response.data.organizations
					} else {
						this.records = []
		...
		delete (ids) {
			this.showLoading('Deleting organization(s), hang on a bit...')
			this.post('/organizations/delete', {
				id: ids
			})
				.then((response) => {
					this.closeLoading()
					if (response.data.success == true) {
						Swal.fire('Deleted!', 'The organization(s) has been deleted.', 'success')
