      ...
      <header class="header-table vs-table--header my-3">
        ...
        >
          <vs-button
            ...
            v-if="canAdd()"
            icon="icon-plus"
            v-on:click="linkto('/accounts/roles/add')"
            >Add Role</vs-button
          >
          <vs-button
            class="mx-2"
          ...
          <table class="vs-table vs-table--tbody-table">
            ...
            <tbody>
              <tr
                ...
                :key="index"
                v-on:click="linkto('/accounts/roles/' + record.id)"
              ...
              >
                ...
                <td>
                  <vs-chip
                    v-for="(permission, index) in record.permissions.split(',')"
                    :key="index"
                ...
                <td>
                  <feather-icon
                    ...
                    v-if="canUpdate()"
                    @click.stop="linkto('/accounts/roles/' + record.id)"
                  />
                  <feather-icon
                    icon="TrashIcon"
...
export default {
  ...
  methods: {
    ...
    delete: function (ids) {
      this.showLoading("Deleting role, hang on a bit...");
      this.post("/users/role_delete", {
        id: ids,
      })
        .then((response) => {
          this.closeLoading();
          if (response.data.success == true) {
            Swal.fire("Deleted!", "The role(s) has been deleted.", "success");
