    ...
    <vx-card ref="filterCard" title="Filters" class="user-list-filters mb-8">
      <div class="vx-row">
        <div class="vx-col md:w-1/4 sm:w-1/2 w-full">
          <label class="text-sm opacity-75">Tax type</label>
          <ajax-select
            ...
            url="/taxtype/options"
            :dir="$vs.rtl ? 'rtl' : 'ltr'"
            :selected="taxtype_filter"
            v-on:update:data="taxtype_filter = $event"
        ...
        <header class="header-table vs-table--header my-3">
          ...
          >
            <vs-button
              ...
              v-if="canAdd()"
              icon="icon-plus"
              v-on:click="linkto('/petroleum/tax-window/add')"
              >Add Tax Window</vs-button
            >
            <vs-button
              class="mx-2"
              color="danger"
            ...
            <table class="vs-table vs-table--tbody-table">
              ...
              <tbody>
                <tr
                  ...
                  :key="index"
                  v-on:click="linkto('/petroleum/tax-window/' + record.id)"
                ...
                >
                  ...
                  <td>
                    <feather-icon
                      ...
                      v-if="canUpdate()"
                      @click.stop="
                        linkto('/petroleum/tax-window/' + record.id)
                      "
                    />
                    <feather-icon
                      icon="TrashIcon"
...
export default {
	...
	data () {
		return {
			pkey: 'tax-window-page-key',
			message: '',
			taxtype: '',
			taxtype_filter: { label: 'All', value: 'all' },
			numbering: 0,
			currentPage: 1,
			result_per_page: 20,
			loading: true,
	...
	methods: {
			...
			this.post('/taxwindow/', {
				...
				.then((response) => {
					...
					if (response.data.success == true) {
						this.message = ''
						this.records = response.data.windows
					} else {
						this.records = []
		...
		delete (ids) {
			this.showLoading('Deleting window(s), hang on a bit...')
			this.post('/taxwindow/delete', {
				id: ids
			})
				.then((response) => {
					this.closeLoading()
