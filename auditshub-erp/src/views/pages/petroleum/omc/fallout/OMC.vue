<template>
  <vx-card title="Oil Marketing Companies">
    <p></p>
          ...
          <table class="vs-table vs-table--tbody-table">
            ...
            <tbody>
              <tr
                ...
                :key="index"
                v-on:click="linkto('/petroleum/omc/fallouts/' + record.id + '/view')"
...
export default {
	...
	data () {
		return {
			pkey: 'omc-fallout-list-page-key',
			message: '',
			numbering: 0,
			currentPage: 1,
			result_per_page: 20,
	...
	methods: {
		...
		empty (ids) {
			this.showLoading('Removing omc(s) receipt, please wait')
			this.post('/omc/empty_receipts', {
				id: ids
			})
				.then((response) => {
					this.closeLoading()
					if (response.data.success == true) {
						Swal.fire(
							'Removed!',
							'The omc(s) receipt has been removed.',
							'success'
						)
						this.selectedRecords = []
					} else {
