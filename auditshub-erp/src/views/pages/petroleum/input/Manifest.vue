<template>
  <div class="cl">
    ...
    >
      <span>Manifest not found. Please add</span>
      <span v-if="AppActiveUser.access_level === 'admin'">
        or
        <span
          @click="popupActive = true"
          class="text-inherit underline cursor-pointer"
          >Upload Manifest</span
        >
      </span>
    </vs-alert>

    <vx-card title="MANIFEST" class="mt-5">
      <p></p>
    ...
    >
      ...
      <p>
        Please select the file containing the <b>Manifest</b> list you wish to
        import.
      </p>
...
export default {
  ...
  data() {
    return {
      ...
      importDetails: "",
      importStatus: "",
      //manifest data list starts here
      pkey: "input-manfest-list",
      message: "",
      numbering: 0,
      currentPage: 1,
      result_per_page: 20,
  ...
  methods: {
      ...
      this.post("/manifest", {
        ...
        .then((response) => {
          ...
          if (response.data.success == true) {
            ...
            this.message = "";
            this.records = response.data.manifest;
          } else {
      ...
      this.post("/manifest/delete", {})
        .then((response) => {
          ...
          if (response.data.success == true) {
            this.$vs.notify({
              title: "Error!!!",
              text: "The Manifest has been deleted.",
              sticky: true,
              border: "danger",
              color: "dark",
              duration: null,
    ...
    checkStatusForImport: function (id) {
      this.post("/manifest/import_status", {
        jobid: id,
      })
        .then((response) => {
