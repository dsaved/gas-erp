...
define('CACHE_DIR',				ROOT . UPLOADS_ADDR . CACHE_DIR_NAME);
class Image{

	public function __construct() {
		$this->http = new Http();
		Files::getInstance()->creat_directory(CACHE_DIR);
	}
	...
	public function show($path){
		if (isset($path)) {
			...
			$image			= preg_replace('/^(s?f|ht)tps?:\/\/[^\/]+/i', '', (string) $path);
			...
			// that we can do. just asign default image to display
			if (!$image || !file_exists($docRoot . $image))
			{
				/*
				http://localhost/bestautobno/library/image.php?crtimage=1&setting=199999_EF5350_1000_200&text=hello%20world  
				setting = 199999_EF5350_1000_200 0-image color. 1-text color. 2-img width. 3-img hieght.

					usage example

				<img src="<?php render(createimage."008080_F_1000_200".text."hello world");?>">
				*/
				$setting = isset($setting) ? $setting : "FFF_111_100_100";
				$setting = explode("_",$setting );

				$width = empty($setting[0]) ? 100 : $setting[2];
				$height = empty($setting[1]) ? 100 : $setting[3];
				$bg_color = empty($setting[2]) ? 100 : $setting[0];
				$txt_color = empty($setting[3]) ? 100 : $setting[1];


				//Define the text to show
				$text = isset($message) ? $message : "NO IMAGE";

				$this->create_image($width, $height, $bg_color, $txt_color, "", $text);
			...
			if(isset($format)){
				...
				foreach ($formats as $format) {
					if (strpos($format, "w")!==false) {	//max width
						$maxWidth=str_replace("w",'',$format);
					}
					if (strpos($format, "h")!==false) {	//max hight
						$maxHeight=str_replace("h",'',$format);
					}
					if (strpos($format, "cr")!==false) { //(optional)ratio of width to height to crop final image (e.g. 1:1 or 3:2 or 5:2)
						$cropratio=str_replace("cr",'',$format);
					}
					if (strpos($format, "ql")!==false) {	//(optional, 0-100, default: 90) quality of output image
						$quality = str_replace("ql",'',$format);
					}
					if (strpos($format, "cl")!==false) {	//(optional) background hex color for filling transparent PNGs (e.g. 900 or 16a942)
						$color = str_replace("cl",'',$format);
					}
					if (strpos($format, "nc")!==false) {	//(optional) does not read image from the cache
						$nocache = str_replace("nc",'nocache',$format);
					}
				}
			}
	...
	public function create_image($width, $height, $bg_color, $txt_color, $font, $text=null)
	{
		if ($text === null) {
			$text = "$width X $height";
		}
		...
		imagettftext($image,$fontsize, 0, ($width/2) - ($fontsize * 2.75), ($height/2) + ($fontsize* 0.2), $txt_color, $font, $text);
