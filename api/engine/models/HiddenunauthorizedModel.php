<?php
class HiddenunauthorizedModel extends BaseModel
{
    ...
    public function getUnauthorized(){
        ...
        if($search){
            ...
            $condition .= " `b`.`bank_name` LIKE '%$search%' OR";
        ...
        if (!empty($condition)) {
            $condition .= " AND ut.`softdelete`=1";
        } else {
            $condition = " WHERE ut.`softdelete`=1";
        }
    ...
    public function reciver_transactions(){
        ...
        $this->paging->result_per_page($result_per_page);
        $this->paging->pageNum($page);
        $this->paging->rawQuery("SELECT ut.id as ut_id, ut.`intval`, ut.`softdelete`, ut.account_from, ut.bog_status,ut.org_status, ut.offset_account, ut.statement_id, ut.time, st.post_date, st.reference, st.debit_amount, st.id AS statement_id FROM `unauthorized_transfers` AS ut LEFT JOIN `statements` AS st ON st.id = ut.statement_id $condition Order By `st`.`post_date` DESC");
        $this->paging->execute();
        $this->paging->reset();
        // var_dump($this->paging);exit;
        $result = $this->paging->results();
        ...
        $this->db->query("SELECT name,acc_num1,acc_num2 FROM `accounts` WHERE `id`={$account_id}");   
        if($this->db->results() && $this->db->count > 0){
            $acct = $this->db->first();
            $response['main_account'] = array(
                "name"=> $acct->name,
    ...
    public function unhide(){
        $id = $this->http->json->id;
        $ids = implode(',', array_map('intval', $id));
        $done = $this->db->query("UPDATE `unauthorized_transfers` SET `softdelete`=0 WHERE statement_id IN ($ids)");
        if ($done) {
            $response['success'] = true;
