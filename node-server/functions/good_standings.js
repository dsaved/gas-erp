module.exports = async function good_standings(sqlConn) {
    ...
    UNION
    SELECT (SELECT date_from FROM tax_window WHERE code=icum_dcl.window_code LIMIT 1) date_from, (SELECT date_to FROM tax_window WHERE code=icum_dcl.window_code LIMIT 1) date_to, icum_dcl.omc omc, SUM(rep.amount) amount, SUM(icum_dcl.amount) dcl_amount_icum,
    SUM(ped.expected_declaration_amount) ped_amount
    FROM ghana_gov_omc_receipt rep RIGHT JOIN petroleum_icums_declaration icum_dcl ON rep.omc=icum_dcl.omc
    RIGHT JOIN petroleum_expected_declaration ped ON rep.omc=ped.omc GROUP BY icum_dcl.omc
    UNION
    SELECT ped.date_from date_from, ped.date_to date_to, ped.omc omc, SUM(rep.amount) amount, SUM(icum_dcl.amount) dcl_amount_icum,
    SUM(ped.expected_declaration_amount) ped_amount
    FROM ghana_gov_omc_receipt rep RIGHT JOIN petroleum_icums_declaration icum_dcl ON rep.omc=icum_dcl.omc
    RIGHT JOIN petroleum_expected_declaration ped ON rep.omc=ped.omc GROUP BY ped.omc, date_from, date_to ORDER BY date_from`).catch(error => {
        console.log(error);
    });
