<?php
class TaxwindowModel extends BaseModel
{
    private static $table = "tax_window";
    ...
    public function get()
    {
        ...
        if (empty($id)) {
            $this->http->_403("Tax window id is required");
        }
        return $this->window(" WHERE `id`= $id");
    }
    
    public function window($condition=" WHERE 1")
    {
        $response = array();
        ...
        if ($search) {
            $condition .= " AND  (`name` LIKE '%$search%' OR `tax_type` IN (SELECT id FROM `tax_type` WHERE `name` LIKE '%$search%') OR `tax_product` IN (SELECT id FROM `tax_schedule_products` WHERE `name` LIKE '%$search%') ) ";
        }
    ...
    public function windows($condition=" WHERE 1")
    {
        $response = array();
        $result_per_page = $this->http->json->result_per_page??20;
        $page = $this->http->json->page??1;
    ...
    public function addtax()
    {
        ...
        if ($date_from===null) {
            $this->http->_403("Please provide tax window date from");
        }

        $date_to = $this->http->json->date_to??null;
        if ($date_to===null) {
            $this->http->_403("Please provide tax window date to");
        }
        ...
        if ($this->db->insert(self::$table, $data)) {
            $response['success'] = true;
            $response['message'] = "Tax Window has been created";
            return $response;
        } else {
            $this->http->_500("Error while creating tax window");
        }
    }
    ...
    public function updatetax()
    {
        ...
        if ($this->db->updateByID(self::$table, "id", $id, $data)) {
            $response['success'] = true;
            $response['message'] = "Tax Window has been updated";
            return $response;
        } else {
            $this->http->_500("Error while updating tax window");
        }
    }
