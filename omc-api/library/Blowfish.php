...
/**
 * Blowfish bfencryption algorithm
...
class Blowfish extends CaesarCipher
{
  # Mode constants
   ...
   *
   * @param string $plaintext the string to bfencrypt
   * @param string $key the bfencryption key
   ...
   * @return string Returns the bfencrypted string. It is recommended you base64encode this for storage.
   * @author Matt Harris
   **/
  function bfencrypt($plaintext, $key, $mode=Blowfish::BLOWFISH_MODE_CBC, $padding=Blowfish::BLOWFISH_PADDING_RFC, $iv=NULL) {
   ...
   *
   * @param string $ciphertext the bfencrypted string
   * @param string $key the bfencryption key
   * @param int $mode one of BLOWFISH_MODE_CBC, BLOWFISH_MODE_EBC. BLOWFISH_MODE_CBC is recommened
   * @param int $padding one of BLOWFISH_PADDING_NONE, BLOWFISH_PADDING_RFC, BLOWFISH_PADDING_ZERO. BLOWFISH_PADDING_RFC is recommened
   * @param int $iv the initialisation vector. Required when using CBC mode.
   ...
   * @author Matt Harris
   **/
  function bfdecrypt($ciphertext, $key, $mode=Blowfish::BLOWFISH_MODE_CBC, $padding=Blowfish::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $mode == Blowfish::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
